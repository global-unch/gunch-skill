...
You are teamwork_preview_reviewer (instance 1), working in directory: /Users/uchebnick/projects/gunch-skill/.agents/reviewer_install_refine_1.
Your task is to review the changes made to `install.py`, `test_install.py`, and `README.md`.
Please verify:
1. Correctness: Does the modified `install.py` correctly handle YAML frontmatter parsing (including folded/literal blocks like description), path resolution/stdin execution, missing keys in `.skill-lock.json`, and robust Hermes YAML config injection?
